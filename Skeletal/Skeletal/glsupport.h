...
inline void safe_glVertexAttribIPointer(const GLint handle, GLint size, GLenum type, GLsizei stride, const GLvoid *pointer) {
  if (handle >= 0)
    glVertexAttribIPointer(handle, size, type, stride, pointer);
}
